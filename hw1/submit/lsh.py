...
def error_measure(A, ks, Ls, verbose=False):
    ret = []
    lsh_times = []
    linear_times = []
    for k in ks:
    ...
    if verbose:
        print("LSH mean time: ", np.mean(lsh_times))
        print("Linear mean time: ", np.mean(linear_times))
        query_index = 99 # FIXME: Ask TA
        plot(A, [query_index], "original")
        lsh_neighbors = lsh_search(A, hashed_A, functions, query_index, 10)
        plot(A, lsh_neighbors, "lsh")
        linear_neighbors = linear_search(A, query_index, 10)
...
def problem4():
    ...
    # final stats
    errors = error_measure(A, [24], [10], verbose=True)
