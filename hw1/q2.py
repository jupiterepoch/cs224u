...
# first pass
candidates = txt.flatMap(parse_line)\
.map(lambda item: (item, 1))\
.reduceByKey(add).filter(lambda item: item[1] >= SUPPORT).collectAsMap()
freq_items = set(candidates.keys())
print(len(candidates))

# second pass
pairs = txt.flatMap(lambda line: get_pair(line, freq_items))\
.map(lambda item: (item, 1))\
.reduceByKey(add).filter(lambda item: item[1] >= SUPPORT).collectAsMap()
...
# third pass
triples = txt.flatMap(lambda line: get_triple(line, freq_items, freq_pairs))\
.map(lambda item: (item, 1))\
.reduceByKey(add).filter(lambda item: item[1] >= SUPPORT).collectAsMap()
# print(triples)
